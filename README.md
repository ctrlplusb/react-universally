...
v13 is going to land fairly soonish via the `next` branch.  It includes a significant amount of updates and restructuring that would make a merged upgrade from v12 painful.  If you want to stay up to date with the project you may want to consider using the `next` branch directly rather than `master`.  The last big outstanding issue for v13 is issue #359.
